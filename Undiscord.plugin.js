...
module.exports = class Undiscord {
  ...
  start() {
    ...
    <br><progress style="display:none; width:24px;"></progress>
      ...
      $("button#getToken").onclick = (e) => {
        ...
        $("input#authToken").value = JSON.parse(win.localStorage.token);
        win.close();
      };
      $("button#getAuthor").onclick = (e) => {
        let wind = window.open();
        window.dispatchEvent(new Event("beforeunload"));
        $("input#authorId").value = JSON.parse(wind.localStorage.user_id_cache);
        wind.close();
      };
  ...
  stop() {
    document.querySelector("#undiscord").destroy();
    document.querySelector("#undiscord-btn").destroy();
